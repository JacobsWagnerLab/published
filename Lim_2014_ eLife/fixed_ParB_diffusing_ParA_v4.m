function [ParB,ParA,tt,params,ParB_dxymax,ParA_state,ParA_0] = fixed_ParB_diffusing_ParA_v4(params)
% UNDER CONSTRUCTION 
% based on diffusing_multiple_ParB_oscillating_ParA_v4 (version='09.10.2014')
% parB do not move
%
% version 11_19_2014:
% - to use the same code to run simulations related to ParA gradient
% formation
% Notation: osciA=parA-Dimer bound to DNA and slowly diffusing
%           freeA=ParA-monomer "doublets" freely diffusing, each represent 2 ParA moelcules, to avoid changing numbers of ParA 
...
D_parA1=params.D_parA1; % D coefficient for DNA-bound ParA
 D_parA2=params.D_parA2; % D coefficient for DNA-bound ParA
...
DdtA1=sqrt(2*dt*D_parA1); DdtA2=sqrt(2*dt*D_parA2);
...
for tt1=dt_out:dt_out:t_fin % "save cycle", save data at each step
  ...
  Dpool_A1=DdtA1*randn(1,2*totA*ceil(dt_out/dt));
  Dpool_A2=DdtA2*randn(1,2*totA*ceil(dt_out/dt));
  ...
  for tt2=dt:dt:dt_out % "silent" cycle, move without saving data
    ...
    d_indx=length(freeA_new);
     if d_indx>0,  
      ...
      % = make moves
     % - ParB  does not move %% Before: ParB moves, if interparticle distance> max_length - apply string
     xB_old=x_B; yB_old=y_B;
     x_B=xB_old; y_B=yB_old;
% %Before:
...
%       y_A(freeA)=x_A(freeA);
     ...
     if d_indx>0
      ...
      x_new=x_A(freeA)+Dpool_A1(1,ddA+1:ddA+d_indx); %- k_xA*deltaXo-k_xA2*deltaXo-k_xA*Dpool_A(2,ddA+1:ddA+d_indx);
       y_new=y_A(freeA)+Dpool_A1(1,ddA+d_indx+1:ddA+2*d_indx); %- k_yA*deltaYo-k_yA2*deltaYo-k_yA*Dpool_A(2,ddA+d_indx+1:ddA+2*d_indx);
      x_A(freeA)=(sign(l00-abs(x_new))+1).*x_new/2+(sign(abs(x_new)-l00)+1).*(l0-abs(x_new)).*sign(x_new)/2;
       y_A(freeA)=(sign(w00-abs(y_new))+1).*y_new/2+(sign(abs(y_new)-w00)+1).*(w0-abs(y_new)).*sign(y_new)/2;
     end
     ddA=ddA+2*d_indx;
     % dimer ParA difusses slowly
     d_indx=length(osciA);
     if d_indx>0
      %deltaXo=(x_A(osciA)-x_A0(osciA));
      % deltaYo=(y_A(osciA)-y_A0(osciA));
      x_new=x_A(osciA)+Dpool_A2(1,ddA+1:ddA+d_indx); %- k_xA*deltaXo-k_xA2*deltaXo-k_xA*Dpool_A(2,ddA+1:ddA+d_indx);
       y_new=y_A(osciA)+Dpool_A2(1,ddA+d_indx+1:ddA+2*d_indx); %- k_yA*deltaYo-k_yA2*deltaYo-k_yA*Dpool_A(2,ddA+d_indx+1:ddA+2*d_indx);
...
% declare default values of parameters and change them to values specified in params_in 
  ...
  params0.D_parA1=10.; % D coefficient for DNA-bound ParA
   params0.D_parA2=0.1; % D coefficient for DNA-bound ParA
  ...
  params0.k_hyd=0.03; % rate of ParB-stimulated hydrolysis, s. Results in ParA dissociation from DNA
   params0.k_db=0.03; % rate of ParA binding to DNA, s. Combined with ATP/ADP-exchange and ParA dimerization
  ...
  params0.model='fixed ParB, diffusing ParA';
   params0.version='11.19.2014';
