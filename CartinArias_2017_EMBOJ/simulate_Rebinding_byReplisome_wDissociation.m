function [X_A,X_R,TT,params_out]=simulate_Rebinding_byReplisome_wDissociation(X_A0,params)
% script to simulate redistribution of DNA-binding protein ("A") that is 
%  (1) reversibly binds to DNA
...
%  (4) or dissociates from the DNA spontaneously with rate k_d
...
%  (4) diffociation is a stochastic process with exponentially distributed
%      time with mean tau=1/k_d 
%  (5) replisome is a "point" (),i.e. size is=0
%
% INPUT:
...
%   .k_d - rate constant of dissociation from DNA 
%
% OUTPUT:
...
ccT=params.ccT;  % cell cycle time, G1, S, and G2/M, min
 ...
 k_d=params.k_d; % DNA-dissociation rate

%=====================
% INITIALIZATION

t_g1=ccT(1); % breaking down cycle into individual periodes G1, S and M
 t_s=ccT(2); % those are times at which these stages end
 t_m=ccT(3);
 tau=sum(ccT); % whole cell cycle
tau_d=1/k_d; % mean time of dissociation 
...
TA=tau*rand(n_A,1); %times of new A appearance
 TA=sort(TA);
TAdiss=tau_d*exprnd(tau_d,[n_A,1]);  %times of A dissociation
...
for  tt=dt:dt:t_g1
  ...
  if n_Anew>0
    ...
    X_A(ii,:)=[X_Aii,X_Anew];
    TAdiss=[TAdiss;tt+tau_d*exprnd(tau_d,[n_Anew,1])]; 
  else
    X_A(ii,:)=X_Aii;   
  end
    % and re-distributing disociated A
  ind_Adiss=((TAdiss>tt-dt) & (TAdiss<=tt));
   n_Adiss=sum(ind_Adiss);
  if n_Adiss>0
    X_Adiss=rand(1,n_Adiss);
    X_A(ii,ind_Adiss)=X_Adiss; % adding -999 to "non-born" A
    TAdiss(ind_Adiss)=tt+tau_d*exprnd(tau_d,[n_Adiss,1]);
  end   
...
for  tt=t_g1+dt:dt:t_g1+t_s
  ...
  ind_Adiss=((TAdiss>tt-dt) & (TAdiss<=tt));
   n_Adiss=sum(ind_Adiss);
  if n_Adiss>0
    X_Adiss=rand(1,n_Adiss)*(1+X_R(ii))-X_R(ii);
    X_A(ii,ind_Adiss)=X_Adiss; % adding -999 to "non-born" A
    TAdiss(ind_Adiss)=tt+tau_d*exprnd(tau_d,[n_Adiss,1]);
  end 
...
for  tt=t_g1+t_s+dt:dt:tau
  ...
  ind_Adiss=((TAdiss>tt-dt) & (TAdiss<=tt));
   n_Adiss=sum(ind_Adiss);
  if n_Adiss>0
    X_Adiss=rand(1,n_Adiss)*(1-L2)+L2;
    X_A(ii,ind_Adiss)=X_Adiss; % adding -999 to "non-born" A
    TAdiss(ind_Adiss)=tt+tau_d*exprnd(tau_d,[n_Adiss,1]);
  end
...
params_out.dt=dt;
 ...
 params_out.n_A=n_A;
 params_out.k_d=k_d;
 params_out.dist=dist;
params_out.script=script;
 params_out.version=version;
 params_out.date=date;
