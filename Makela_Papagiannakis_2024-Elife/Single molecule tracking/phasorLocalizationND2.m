function phasorLocalizationND2(imagename, thresh, windowSize)
...
savename = [imagename(1:(end-4)),'_thresh',num2str(thresh),'_frames',num2str(framesLocalization(1)),'to',num2str(framesLocalization(2)),'.phasor.pos.out'];
...
reader = bfGetReader(imagename);

% number of frames if the whole movie is to be analyzed
% this assumes NIS-ELEMENTS output of .nd2 to find number of frames
if framesLocalization(2) == inf  
    % find OME metadata
    omeMeta = reader.getMetadataStore();
    % convert to char
    omeXML = char(omeMeta.dumpXML());
    % take only beginning of the metadata
    omeShort = omeXML(1:3000);
    % split the string to get out number of T frames (SizeT)
    splitted = strsplit(omeShort,{'SizeT="','" SizeX'});
    framesLocalization(2) = str2num(splitted{2});
end
...
while carryOn && ii<=framesLocalization(2) % for each specified frame
    ...
    try im = double(bfGetPlane(reader, ii));
    catch ME
        carryOn = 0;
    end
    
    if carryOn
        % filter and find local maxima
