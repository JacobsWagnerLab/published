...
| Script | Purpose |
|---|---|
| `compute_kymo_growth_rate_wide-trench.py` | From curated single-cell trajectories, estimate instantaneous growth rates, bin the cell center-of-mass y-coordinate into 12-px (0.79 µm) bins, prune isolated bins with an iterative neighborhood filter, and build median-normalized-growth-rate kymographs over time × position. Pools across trenches/replicates. |
