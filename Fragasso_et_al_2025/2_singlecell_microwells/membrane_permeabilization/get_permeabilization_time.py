...
def get_permeabilization_time(df, ch_info_df, ch_info_list = ['mScarlet','BFP','Sytox'],   w_Sy = 5, fr = 1, peri_fluor = 'fluor1', thr_IM = -25, t_max = 241):
    ...
    for cell in df['cell_id'].unique().tolist():
        ...
        for col in ['tp_IM','tp_aligned_IM','fp_IM','pc_diff_IM']:
            tp_temp[col] = np.nan
        ...
        for i in range(len(ch_info_list)):
            ...
            if ch_info_list[i] == 'Sytox':                  ## probe permeabilization to Sytox Green
                ...
                if len(valid_peaks)>0 and cell_df['time_min_aligned'].iloc[idx]>5:
                    idx = valid_peaks[0] -cell_df.index.tolist()[0]
                    if not np.isnan(tp_temp['pc_diff_IM'].iloc[0]) and ch_info_list[i] == 'BFP':    # IM perm was detected before OM and I'm detecting BFP loss
                    ...
                    elif np.isnan(tp_temp['pc_diff_IM'].iloc[0]):                           # IM perm was not detected
        ...
        if not np.isnan(tp_temp['tp_IM'].iloc[0]):  # if tp_IM was detected
            if np.isnan(tp_temp['pc_diff_IM'].iloc[0]) and not 'tp_OM' in tp_temp.columns:   # if tp_IM was detected not through peri_cyto_diff, but tp_OM was not detected, then that was not a IM perm. event
                for col in ['tp_IM','tp_aligned_IM','fp_IM','pc_diff_IM']:   # in that case, reset all IM detection timings to nan
                    tp_temp[col] = np.nan
            elif tp_temp['tp_IM'].iloc[0] < tp_temp['t_gr_stop'].iloc[0]-15 or tp_temp['tp_aligned_IM'].iloc[0]<5:
                for col in ['tp_IM','tp_aligned_IM','fp_IM','pc_diff_IM']:   # in that case, reset all IM detection timings to nan
                    tp_temp[col] = np.nan

        if not np.isnan(tp_temp['tp_IM'].iloc[0]):
