...
def extract_cell_data(mask, phase, fluor1, fluor2, cell_ch_dict, frame_number, frame_interval, cell_id_counter,
    ...
    for prop in props:
        ...
        if show:
            contours_ero = find_contours(mask_eroded, level=0.5)          # FIX: was measure.find_contours (measure not imported)
            contours = find_contours(cropped_mask, level=0.5)
            fig = plt.figure(figsize=(25, 10))
            fig.suptitle(cell_id + ', frame = ' + str(frame_number))       # FIX: was str(k) (k not in scope)
