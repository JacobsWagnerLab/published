function [filteredIM] = filterIM(im, varargin)
...
b_im = conv2(double(im), norm(ones(spotLength)),'same');

%pad the image for use with fft2
[r_ind, c_ind] = size(im);
Fp = cat(1, im, zeros(r_ind, c_ind));
Fp = cat(2, Fp, zeros(size(Fp,1), c_ind));
Fuv = fft2(Fp);

%construct a gaussian filter and pad with 0s for fft2
...
%extract the valid filtered region
...
L = (ithresh)*(max(filteredIM(:)) - min(filteredIM(:)))*graythresh(filteredIM);
filteredIM(filteredIM <= L) = 0;

if nargout == 0
    imshow(filteredIM,[min(filteredIM(:)) max(filteredIM(:))],'InitialMagnification','fit')
end
