...
% This code collect all "siblings" (daughter cell pairs)
% in the tree structure and generate data structure "sibling_stat"
...
for DB = 1:length(dataset_name)
    ...
    sibling_stat{DB}.data = triad_statistics(triad0, tag_array);
    sibling_stat{DB}.dataset_name = dataset_name{DB};
...
for j = 1:SBL_num
    ...
    output(j,1) = data.triad_dataset{j}.ccB.(tag1).(tag2);
    output(j,2) = data.triad_dataset{j}.ccC.(tag1).(tag2);
    output(j,3) = data.rec(j,5);
    output(j,4) = data.rec(j,6);
...
for j = 1:SBL_num
    ...
    if (output(j,3) > output(j,4))  % revert the data order
        
        temp = [output(j,2) output(j,1) output(j,4) output(j,3)];
        output(j,:) = temp;
    end
