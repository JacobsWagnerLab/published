...
qt_cut: The quantile cutoff threshold for the 1D segment, ranged from 0 to 1.  
        The larger the number, the more likely to cut cells. 
...
%     where first column are starting points of 0-segments
%     and the second column are ending points of 0-segments
...
% Except for the first and last segment use the "tight region" 

c_ind = NaN(size(ind_mat,1), 1);

for m = 1:size(ind_mat,1)

    if (m == 1)
        c_ind(m) = max(1, ind_mat(m,2)-1);
        
    elseif (m > 1) && (m < size(ind_mat,1) )
        
        c_ind(m) = ( ind_mat(m,1) + ind_mat(m,2) ) / 2;
        c_ind(m) = floor(c_ind(m));

    elseif ( m == size(ind_mat,1) )
        
        c_ind(m) = min(L, ind_mat(m,1)+1);
