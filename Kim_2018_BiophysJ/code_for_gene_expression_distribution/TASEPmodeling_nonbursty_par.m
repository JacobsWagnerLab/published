function ans = TASEPmodeling_nonbursty_par(pauseProfile,promoter,parRun,kLoading,avgSpeed,pauseSite, pauseHeight)
%{
-About-
...
mRNA number, protein number, temporal profile of protein production
...
-Example-
TASEPmodeling_nonbursty_par('flat','P1nb',1,1/15,30,0,0)
...
elseif strcmp(pauseProfile, 'MultipauseAbs')
    specificDwelltime = avgDwelltime * ones(geneLength/dx,1); % no pause, flat profile
    specificDwelltime(pauseSite) = pauseHeight;
    runCondition = strcat(pauseProfile,sprintf('%01.0f',length(pauseSite)),'x',sprintf('%01.0f',pauseHeight(1)),'x',sprintf('%01.0f',pauseHeight(2)),'-NO-');
end;
...
save(fileName,'specificDwelltime','loadingSample','presentonDNA','fishSignal1','fishSignal2','fishSignal3',...
    'riboNumHistFine','rEndStampA','proteinLoci2',...
