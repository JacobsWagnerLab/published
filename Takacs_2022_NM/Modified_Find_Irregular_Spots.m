%{
-About-
This function detects foci with various properties, including
diffraction-limited foci and foci with irregular features. It is comparable
to spotDetection and spotFinder, but its performance is more robust,
particularly for identifying irregular spots. A case where this code was
used is in Arias-Cartin, et al. EMBO J, 2017.

Finds local peaks and then constructs a shell of user determined thickness
at radius away from bright pixels. The intensity of the spot is measured
as the ratio of the peak intensity to the intensity of pixels within the
shell.
...
-Inputs-
    ...
    images -            A cell array of image locations such that images{1}
                        corresponds to the frame at cellList.meshData{1}. I
                        produce this by importing my fluorescence images
                        using the function loadimseries

    frames -            The number of frames from which you want to detect
                        spots. To detect spots in all frames, you can write
                        length(cellList.meshData). Decreasing the number of
                        frames that you check is helpful to test parameters
                        for the spot detection without taking as much time.

-varargin-

    peakRadius -        a pixel will only be considered a local maxima if
                        it is the brightest peak within radius defined by
                        peakRadius

    edgeDist -          Radius from the peak where the outter edge of the
                        shell will be constructed
    
    shellThickness -    thickness of the shell constructed at edgeDist to
                        be used in intensity ratio calculation

    centerDist -        Radius from the peak that will be used to calculate
                        peak intensity. Intensity of all pixels within
                        centerDist from the peak will be used in the
                        intensity ratio calculation. Should typically be set
                        to small values (i.e., 1)

    intensityRatioThreshold -
                        if the ratio if peak to shell intensity is less
                        than intensityRatioThreshold, the spot will be
                        discarded

    quantileThreshold - pixels must be greater than quantileThreshold in the
                        spot to be fit, else they will be ignored even if
                        they are within fitRadius. This permits a large fit
                        radius to be selected for irregular spots. Pixels to
                        fit can then be chosen based on intensity by
                        modulating quantileThreshold

-Outputs-

    cellList -          Returns a cellList, but with your added spots
                        field, containing information about the spots. This
                        information is found in three fields of cellList:
                        
                        spotPosition: x,y coordinate in the frame where the
                        spot is found

                        intensityRatio: ratio between the center's
                        intensity and the intensity of the outer shell used
                        in detecting the spot

                        spotIntensity: Summed intensity of the spot
...
-Dependencies-
This function contains three internal sub-functions:

    morphCompFilter -   use a morphological element to perform isotropic
                        pixel comparison according to the operation
                        specified by operator

    ballElement -       a structuring element of size sz will be constructed
                        so that all pixels within radius of the center will
                        be set to 1 and all pixels of distance > radius
                        from center will be 0

    g2d -               calculate the centroid, weighted mean of a 2d matrix

These sub-functions have been built in to the broader function for
simplicity.

-References-
Arias-Cartin, et al. EMBO J, 2017

-Author-
Brad Parry (original), 2017
...
%-------------------------------------------------------------------------
%-------------------------------------------------------------------------
function cellList = Modified_Find_Irregular_Spots(cellList, images, frames, varargin)
...
shellThickness = 1;
quantileThreshold = .3;
intensityRatioThreshold = 1.3;

%check through the inputs to see if the user has specified values to use
...
for F = 1:frames
    ...
    for C = 1:length(cellList.meshData{F})
        ...
        if isempty(cellList.meshData{F}{C}) || ~isfield(cellList.meshData{F}{C},'mesh') ||...
                length(cellList.meshData{F}{C}.mesh) < 6
            %you know the drill...
            continue
        end
        im = images(:,:,F);
        imsize = size(im); %in principle, this could be done outside the loop, assuming
        ...
        for k = length(r):-1:1
            ...
            if ~isfield(cellList.meshData{F}{C},'spotPosition')
                cellList.meshData{F}{C}.spotPosition = [];
                cellList.meshData{F}{C}.intensityRatio = [];
                cellList.meshData{F}{C}.spotIntensity = [];
            end
            
            cellList.meshData{F}{C}.spotPosition(end+1,:) = pos(end,:);
            cellList.meshData{F}{C}.intensityRatio(end+1) = intensityRatio(k);
            cellList.meshData{F}{C}.spotIntensity(end+1) = sum(spotOrig(:));
