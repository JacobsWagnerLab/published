function [cellList] = Add_FtsZ_Characteristics(cellList, path_to_images)
%{
-About-
Function to extract FtsZ-Venus signal information.
More specifically, the relative intracellular area of fluorescently
labeled FtsZ. 
The fields 'relative_FtsZ_area' is added for each cell in the cellList.

-Inputs-
cellList:  cellList generated by Oufti, loaded into matlab. 
            
path:   full path to folder containing FtsZ fluorescence images. This
...
-Outputs-
cellList:   updated cellList containing FtsZ information, the fields 
            'relative_FtsZ_area' is added for each cell in the cellList.
...
-Keywords-
FtsZ, Relative FtsZ area, cellList, Oufti
...
%% Extract FtsZ information

ftsZ_images = loadimseries(path_to_images);
%Display progress bar
w = waitbar(0,'Extracting FtsZ information');


%Loop through frames included in the cellList
for ii = 1:length(cellList.meshData)
    %Select corresponding ftsZ image
    ftsZ_single_image = ftsZ_images(:,:,ii); 
    %Run filterIM function to enhance features in fluorescent image
    filtered_image = filterIM(ftsZ_single_image,'fq',1,'sl',10,'ithresh',0);
    ...
    for jj=1:length(cellList.meshData{ii})
        ...
        if ~isempty(cellList.meshData{ii}{jj})
            ...
            %remains
            cropped_image = imcrop(ftsZ_single_image, crop_region);
            ...
            %Decrease threshold size to include more of the FtsZ signal
            ...
            %use regionprops function to extract properties of FtsZ and
            %the corresponding cell
            ftsZ_stats = regionprops(bw_cropped_image,cropped_image,'Area','PixelValues');
            cell_stats = regionprops(cell_mask,cropped_image,'Area','PixelValues');

            cellList.meshData{ii}{jj}.relative_ftsZ_area=[];
            %Only calculate the relative FtsZ area if the properties for
            %the ftsZ signal were extracted
            if ~isempty(ftsZ_stats)
                %Due to occassional issues (<0.1 %) with the regionprops function for these images,
                %use try statement to calculate the relative FtsZ area and
                %write it into the cellList
                try
                    cellList.meshData{ii}{jj}.relative_ftsZ_area=ftsZ_stats.Area/cell_stats.Area;
                %If calculation fails, set relative area to NaN
                catch
                    cellList.meshData{ii}{jj}.relative_ftsZ_area=NaN;
                end
            end
        end
    end
