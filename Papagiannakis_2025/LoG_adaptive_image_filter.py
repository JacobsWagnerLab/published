# -*- coding: utf-8 -*-
"""
Created on Thu Mar 21 10:58:56 2024

@author: Alexandros Papagiannakis, Christine Jacobs-Wagner lab, Stanford University / HHMI
"""
...
def log_adaptive_filter(image, parameters): 
    ...
    image_smoothed = sp.ndimage.gaussian_filter(image, sigma = parameters[0])
    image_laplace = skimage.filters.laplace(image_smoothed, parameters[1])
    ...
    image_smoothed_2 = sp.ndimage.gaussian_filter(image, sigma=parameters[3])
    adaptive_threshold = skimage.filters.threshold_local(image_smoothed_2, block_size=parameters[4], offset=parameters[5])
    ...
    if parameters[6] > 0:
        adaptively_masked_image = sp.ndimage.morphology.binary_erosion(masked_image, iterations = parameters[6])
