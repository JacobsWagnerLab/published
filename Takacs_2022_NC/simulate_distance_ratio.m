%% Simulate the distance ratios
% -Input-
%   cellList: Oufti cellList with 'che' and/or 'gfp' fields
%   Optional 2nd argument: the number of simulation repeats
% -Output-
%   simulation_ratios: a struct with 4 different fields:
%       1. che_include_edge: simulation based on the che puncta, distance
%       ratios calculated based on the entire cell length
%       2. che_exclude_edge: simulation based on the che puncta, distance
%       ratios calculated between the 1st and last punctum
%       3. gfp_include_edge: simulation based on the gfp puncta, distance
%       ratios calculated based on the entire cell length
%       4. gfp_exclude_edge: simulation based on the gfp puncta, distance
%       ratios calculated between the 1st and last punctum
...
%       stats on how much the distance ratios deviate from uniform
%       distribution, calculated between the 1st and last punctum
function simulation_ratios = simulate_distance_ratio(cellList,varargin)
if nargin == 1
    repeat = 1;
else
    repeat = varargin{1};
...
for frame = 1:length(cellList.meshData)
    ...
    for cc = 1:length(cellList.meshData{frame})
        if isfield(cellList.meshData{frame}{cc},'che') && ...
            ...
            ratio_che_include_edge{cc} = sim_distance_ratio(cellList.meshData{frame}{cc},'che',true,repeat);
            ratio_che_exclude_edge{cc} = sim_distance_ratio(cellList.meshData{frame}{cc},'che',false,repeat);
        ...
        if isfield(cellList.meshData{frame}{cc},'gfp') && ...
            ...
            ratio_gfp_include_edge{cc} = sim_distance_ratio(cellList.meshData{frame}{cc},'gfp',true,repeat);
            ratio_gfp_exclude_edge{cc} = sim_distance_ratio(cellList.meshData{frame}{cc},'gfp',false,repeat);
...
simulation_ratios.che_include_edge = vertcat(all_ratio_che_include_edge{:});
simulation_ratios.che_exclude_edge = vertcat(all_ratio_che_exclude_edge{:});
simulation_ratios.gfp_include_edge = vertcat(all_ratio_gfp_include_edge{:});
simulation_ratios.gfp_exclude_edge = vertcat(all_ratio_gfp_exclude_edge{:});

simulation_ratios.che_deviation_stats_include_edge = vertcat(all_che_stats_include_edge{:});
simulation_ratios.che_deviation_stats_exclude_edge = vertcat(all_che_stats_exclude_edge{:});
simulation_ratios.gfp_deviation_stats_include_edge = vertcat(all_gfp_stats_include_edge{:});
simulation_ratios.gfp_deviation_stats_exclude_edge = vertcat(all_gfp_stats_exclude_edge{:});
...
% Helper function for calculating the distance
    ...
    function ratios = sim_distance_ratio(cellStruct,puncta_color,include_edge,repeat)
        ratios = cell(repeat,1);
        for ii = 1:repeat
            number_of_puncta = size(cellStruct.(puncta_color).spotPosition_rel,1);
            simulated_positions = rand(number_of_puncta,1).*cellStruct.length;
            separation = calculate_interpuncta_distance(simulated_positions);
            if include_edge
